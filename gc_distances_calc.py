...
in_file=open('gc_network.csv','r')
out_file=open('gc_distances.txt','w')
...
for line in in_file:
    ...
    G.add_edge(line[0],line[1],weight=float(line[2][:-1]))
...
#    print (G.nodes())
    ...
    for nod_i in giant.nodes():
        ...
        i=i+1
        j=0
        for nod_j in giant.nodes():
            j=j+1
            if i==j:
                continue
                       
            short_path=nx.shortest_path(giant,source=nod_i,target=nod_j)
