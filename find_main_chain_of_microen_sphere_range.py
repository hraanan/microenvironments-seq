...
def get_main_chain(microen):
    ...
    try:
                ...
                if len(neighbors_chain_list)>1:
                    ...
                    for res in all_neighbors:
                        if res.resname in AA:
                            #print(res)
                            neighbors_res_list.append([aa_dict.get(res.resname),res.id[1]])
        ...
        if multi==False:
            chain = model[neighbors_chain_list[0]]
            min_res=1000000
            max_res=0
            sphere_res_range=[0,0]
            for residue in chain:
                ...
                #print(residue.id)
                    if residue.id[1]<min_res:
                        min_res=residue.id[1]
                    if residue.id[1]>min_res:
                        max_res=residue.id[1]
            sphere_res_range=[min_res,max_res]
            for residue in chain:
                if residue.id[1]>=neighbors_res_range[0] and residue.id[1]<=neighbors_res_range[1]:
                    neighbors_res_seq=neighbors_res_seq+aa_dict.get(residue.resname)
                if residue.id[1]>=sphere_res_range[0] and residue.id[1]<=sphere_res_range[1]:
                    sphere_res_seq=sphere_res_seq+aa_dict.get(residue.resname)    
...
#    dash_seq=[]
#    for i in range(0,int(neighbors_res_range[1]+1)):
#        
#        dash_seq.append(random.choice(aa))
#    #print(len(dash_seq))
#    new_list=[]
#    for res in neighbors_res_list:
#        
#        dash_seq[int(res[1])]=res[0]
#        new_list.append(res[0]+','+str(res[1]))
#    neighbors_res_list=new_list
    #print("".join(dash_seq))
    
    return [multi,neighbors_chain_list,neighbors_res_range,sphere_res_range,neighbors_res_seq,sphere_res_seq,neighbors_res_list,"".join(dash_seq)]
